...
const Jugadores = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error("Error players-top:", response.status, response.statusText);
      return null;
    }
    ...
    if (!contentType?.includes("application/json")) {
      console.error("players-top no devolvió JSON:", contentType);
      return null;
    }

    data = (await response.json()) as Data[];
  } catch (error) {
    console.error("Error obteniendo players-top:", error);
    return null;
  }
